...
/// [EntLibCryptoRegistry]에서 모든 번들의 스트레티지를 수집하여 사용합니다.
/// ## 사용법
///
/// ```java
...
public abstract class AbstractStrategyBundle implements RegistrableStrategy {
     ...
     */
    private final Map<EntLibAlgorithmType, EntLibCryptoStrategy> algStrategies = new ConcurrentHashMap<>();

    private final Map<EntLibAlgorithmType, EntLibKey> keyStrategies = new ConcurrentHashMap<>();
     ...
     * <p>
     * {@link #register(EntLibAlgorithmType, EntLibCryptoStrategy, EntLibKey)} 메소드를 사용하여 등록합니다.
     */
     ...
     *
     * @param type        알고리즘 타입
     * @param algStrategy 등록할 알고리즘 스트레티지
     * @param keyStrategy 등록할 키 스트레티지
     */
    protected final void register(@NotNull EntLibAlgorithmType type,
                                  @NotNull EntLibCryptoStrategy algStrategy,
                                  @NotNull EntLibKey keyStrategy) {
        algStrategies.put(type, algStrategy);
        keyStrategies.put(type, keyStrategy);
    }
     ...
     */
    @Override
    public final Map<EntLibAlgorithmType, EntLibCryptoStrategy> getAlgStrategies() {
        if (!initialized) {
            synchronized (this) {
                if (!initialized) {
                    registerStrategies();
        ...
        return Collections.unmodifiableMap(algStrategies);
    }

    /**
     * 이 번들의 모든 키 스트레티지를 반환하는 메소드입니다.
     * <p>
     * 처음 호출 시 {@link #registerStrategies()}가 실행됩니다.
     *
     * @return 등록된 스트레티지 맵 (읽기 전용)
     */
    @Override
    public final Map<EntLibAlgorithmType, EntLibKey> getKeyStrategies() {
        if (!initialized) {
            synchronized (this) {
                if (!initialized) {
                    registerStrategies();
        ...
        return Collections.unmodifiableMap(keyStrategies);
    }
     ...
     */
    public static Map<EntLibAlgorithmType, EntLibCryptoStrategy> collectAllAlgStrategies() {
        ...
        for (AbstractStrategyBundle bundle : REGISTERED_BUNDLES) {
            result.putAll(bundle.getAlgStrategies());
        }
        return result;
    }

    /**
     * 등록된 모든 번들에서 키 스트레티지를 수집하여 반환하는 메소드입니다.
     *
     * @return 모든 번들의 키 스트레티지를 합친 맵
     */
    public static Map<EntLibAlgorithmType, EntLibKey> collectAllKeyStrategies() {
        Map<EntLibAlgorithmType, EntLibKey> result = new ConcurrentHashMap<>();
        for (AbstractStrategyBundle bundle : REGISTERED_BUNDLES) {
            result.putAll(bundle.getKeyStrategies());
        }
        return result;
    }
