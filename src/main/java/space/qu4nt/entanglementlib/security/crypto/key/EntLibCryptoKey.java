...
/// @see KeyWiper
/// @since 1.1.0
/// @deprecated 이제 민감 데이터는 [SensitiveDataContainer] 클래스가 소유합니다.
@Slf4j
@Deprecated
public class EntLibCryptoKey implements AutoCloseable {
    ...
    public void wipe() {
        if (arena.scope().isAlive()) {
            ...
            try {
                // entlib-native 함수 호출
                InternalFactory.callNativeLib().getHandle("entanglement_secure_wipe")
                        .invokeExact(keySegment, keySegment.byteSize());
