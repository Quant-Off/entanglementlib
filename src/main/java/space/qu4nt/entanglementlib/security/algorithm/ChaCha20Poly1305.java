 ...
 */
@Slf4j
public final class ChaCha20Poly1305 implements SymmetricCryptoService, StreamingCryptoService {

    private static final LanguageInstanceBased<ChaCha20Poly1305> lang = LanguageInstanceBased.create(ChaCha20Poly1305.class);
    private final ClassicalType type = ClassicalType.CHACHA20_POLY1305;
    ...
    /**
     * ChaCha20-Poly1305 알고리즘에 사용되는 설정 정보를 담는 클래스입니다.
     * <p>
     ...
     * @since 1.0.0
     */
    public static final class ChaCha20Poly1305Setting {
        ...
        @lombok.Builder
        public ChaCha20Poly1305Setting(String plain, byte[] plainBytes, int nonceSize, int chunkSize, @Nullable Path encryptedOutput, @Nullable Path decryptedOutput) {
            if (plainBytes != null) {
                this.plainBytes = plainBytes.clone();
            } else if (plain != null) {
                this.plainBytes = plain.getBytes(StandardCharsets.UTF_8);
            } else {
                throw new EntLibAlgorithmSettingException(ChaCha20Poly1305.class, "plaintext-or-byte-array-exc");
            }
            this.type = ClassicalType.CHACHA20_POLY1305;
    ...
    private ChaCha20Poly1305(byte[] nonce, @NotNull String plain, @Nullable Path encryptedOutput, @Nullable Path decryptedOutput) {
        log.debug(lang.setClass(EntLibCryptoService.class)
                .argsNonTopKey("debug-created-instance", "ChaCha20-Poly1305"));
        this.nonce = nonce;
        this.plainBytes = plain.getBytes(StandardCharsets.UTF_8);
        this.encryptedOutput = encryptedOutput;
        this.decryptedOutput = decryptedOutput;
    ...
    private ChaCha20Poly1305(byte[] nonce, byte @NotNull [] plainBytes, @Nullable Path encryptedOutput, @Nullable Path decryptedOutput) {
        log.debug(lang.setClass(EntLibCryptoService.class)
                .argsNonTopKey("debug-created-instance", "ChaCha20-Poly1305"));
        this.nonce = nonce;
    ...
    public static ChaCha20Poly1305 create(byte[] nonce, @NotNull String plain) {
        return new ChaCha20Poly1305(nonce, plain, null, null);
    }

    public static ChaCha20Poly1305 create(byte[] nonce, byte @NotNull [] plainBytes) {
        return new ChaCha20Poly1305(nonce, plainBytes, null, null);
    }

    public static ChaCha20Poly1305 create(@NotNull ChaCha20Poly1305.ChaCha20Poly1305Setting chaCha20Poly1305Setting) {
        return new ChaCha20Poly1305(chaCha20Poly1305Setting.getNonce(), chaCha20Poly1305Setting.getPlainBytes(), chaCha20Poly1305Setting.getEncryptedOutput(), chaCha20Poly1305Setting.getDecryptedOutput());
    }
    ...
    public EntLibParameterSpec getType() {
        return ClassicalType.CHACHA20_POLY1305;
    }
    ...
    public byte[] encrypt(@NotNull SecretKey secretKey, byte @NotNull [] plainBytes, @Nullable Padding padding, int chunkSize)
        ...
        AlgorithmParameterSpec mamboSpec = new IvParameterSpec(nonce);
    ...
    public void close() throws Exception {
        ...
        closed = true;
        log.debug(LanguageInstanceBased.create(EntLibCryptoService.class).argsNonTopKey("debug-instance-closed", "ChaCha20-Poly1305"));
    }

    private void checkClosed() {
        if (closed) {
            throw new EntLibSecureIllegalStateException(EntLibCryptoService.class, "data-already-destroyed-exc", "ChaCha20-Poly1305");
        }
    }
}
