# EF: BC Lightweight API

이 패키지는 `BouncyCastle` 라이브러리를 기반으로 한 암호화, 전자 서명, 키 관리 기능을 제공하는 `EntLib`의 실험적 암호화 모듈입니다.

이 기능은 `exp-bc-lightweight-api` 브랜치에서 공개되며 `1.1.0` 릴리즈에 포함됩니다.

## 주요 기능

* 암호화 알고리즘 지원: AES, ARIA, ChaCha20 등 다양한 대칭 키 암호화 알고리즘을 지원합니다.
* 전자 서명 지원: RSA 및 차세대 PQC(Post-Quantum Cryptography) 알고리즘인 ML-DSA를 지원합니다.
* 확장 가능한 아키텍처: `AbstractStrategyBundle` 및 `EntLibCryptoRegistry`를 통해 새로운 알고리즘을 쉽게 추가하고 관리할 수 있습니다.
* 안전한 키 관리: `EntLibCryptoKey` 및 `KeyWiper`를 통해 암호화 키의 생성, 저장, 폐기를 안전하게 처리합니다.
...
* `AESStrategyBundle`: AES-128, AES-192, AES-256 스트레티지를 등록하는 번들입니다.
* `ARIAStrategyBundle`: ARIA-128, ARIA-192, ARIA-256 스트레티지를 등록하는 번들입니다.
* `MLDSAStrategyBundle`: ML-DSA-44, ML-DSA-65, ML-DSA-87 서명 스트레티지를 등록하는 번들입니다.
...
`EntLibCryptoKey`를 사용하여 키를 안전하게 관리합니다.

```java
byte[] rawKey = ...; // 원본 키 데이터
try (EntLibCryptoKey key = new EntLibCryptoKey(rawKey)) {
    // 키 사용
    // ...
} // try 블록 종료 시 자동으로 키 메모리 소거 및 해제
```
