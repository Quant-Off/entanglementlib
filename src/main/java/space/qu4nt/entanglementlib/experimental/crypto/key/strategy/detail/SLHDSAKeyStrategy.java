 ...
 * {@link SLHDSAStrategy}와 함께 사용됩니다.
 * </p>
 *
 * @author Q. T. Felix
...
public final class SLHDSAKeyStrategy implements EntLibAsymmetricKeyStrategy {
     ...
     */
    private SLHDSAKeyStrategy(SLHDSAStrategy SLHDSAStrategy) {
        this.slhdsaParameters = SLHDSAStrategy.findInternalParameters();
    }
     ...
     */
    public static SLHDSAKeyStrategy create(@NotNull SLHDSAStrategy SLHDSAStrategy) {
        return new SLHDSAKeyStrategy(SLHDSAStrategy);
    }
    ...
    public Pair<EntLibCryptoKey, EntLibCryptoKey> generateKeyPair() {
        try (final Internal in = new Internal(slhdsaParameters)) {
            return in.gen();
        } catch (Exception e) {
     ...
     * {@link SLHDSAParameters} 클래스 내에 package-private 처리된 엔진 호출 메소드를
     ...
     * @since 1.1.0
     */
    private static class Internal implements AutoCloseable {
        ...
        private Tuple<byte[], byte[], byte[]> calc(boolean sha2) throws ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, NoSuchFieldException {
            ...
            byte[] skSeed = EntLibKey.generateSafeRandomBytes(n);
            byte[] skPrf = EntLibKey.generateSafeRandomBytes(n);
            byte[] pkSeed = EntLibKey.generateSafeRandomBytes(n);
        ...
        @Override
        public void close() {
//            KeyDestroyHelper.destroy(engine);
//            KeyDestroyHelper.zeroing(tuple.getFirst());
//            KeyDestroyHelper.zeroing(tuple.getSecond());
//            KeyDestroyHelper.zeroing(tuple.getThird());
        }
    }
}
