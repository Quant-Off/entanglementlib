...
/// JDK 검증 공급자가 off-heap [SensitiveDataContainer]와 JCA가 요구하는 `byte[]` 사이를 잇기 위한
/// 내부 브리지 헬퍼입니다.
///
/// # Security Note
/// JCA(MessageDigest, Cipher 등)는 `byte[]`로만 동작하므로 검증 백엔드를 선택하면 연산 시간 동안
/// 민감 데이터가 JVM `heap`에 잠시 노출됩니다. 이는 README가 명시한 의도된 절충(검증된 정확성을 위해
/// 짧은 heap 노출을 감수)입니다. 이 헬퍼는 사용한 모든 임시 `byte[]`를 [#wipe]로 즉시 0으로 덮어
/// 잔류 시간을 최소화합니다.
final class SdcCodec {
    ...
    static byte @NotNull [] read(final @NotNull SensitiveDataContainer container) {
    ...
    static @NotNull SensitiveDataContainer write(final @NotNull SDCScopeContext scope, final byte @NotNull [] data)
    ...
    static void wipe(final byte[]... arrays) {
